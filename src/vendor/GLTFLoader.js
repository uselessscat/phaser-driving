...
GLTFMaterialsUnlitExtension.prototype.getMaterialType = (material) => THREE.MeshBasicMaterial;

GLTFMaterialsUnlitExtension.prototype.extendParams = (materialParams, material, parser) => {
	var pending = [];
...
GLTFParser.prototype.loadAccessor = function (accessorIndex) {
	...
	return Promise.all(pendingBufferViews).then((bufferViews) => {
		...
		if (byteStride && byteStride !== itemBytes) {
			...
			var ib = this.cache.get(ibCacheKey);
...
GLTFParser.prototype.loadScene = (() => {
	...
	return function loadScene(sceneIndex) {
		...
		for (var i = 0, il = nodeIds.length; i < il; i++) {
			pending.push(buildNodeHierachy(nodeIds[i], scene, json, this));
		}
